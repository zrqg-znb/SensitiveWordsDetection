...
class SensitiveWord(BaseModel, TimestampMixin):
    word = fields.CharField(max_length=255, description="敏感词", index=True)
    category = fields.CharField(max_length=255, description="分类", index=True)
    level = fields.IntField(default=1, description="敏感级别", index=True)
...
class Crawler(BaseModel, TimestampMixin):
    url = fields.CharField(max_length=255, description="URL地址", index=True)
    ...
    result_url = fields.CharField(max_length=500, description="结果报告URL", index=True)
